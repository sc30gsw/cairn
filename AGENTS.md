...
- **Auth (Cloud Agent / PR testing):** Set `BETTER_AUTH_SECRET` and `SITE_URL=http://localhost:3000` on the Convex deployment; `VITE_SITE_URL=http://localhost:3000` in `.env.local`. Sign up with username + email + password on the login screen, or use Google OAuth when `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` are set (calendar sync additionally needs the Google Calendar API enabled on that OAuth client — see `docs/specs/calendar-sync.md` §6).
