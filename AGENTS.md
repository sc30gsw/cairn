...
- **Seed data:** First owner `days.open` calls `ensureCatalog` (items, weekday presets, exam goal). There is no `tasks` table or `sampleData.jsonl`. Verify with `convex run days:get '{"dateJst":"2026-08-17","todayJst":"2026-08-17"}'` after signing in, or via `vp test`.
- **Dev email auth (PR / cloud testing):** Set `ENABLE_DEV_EMAIL_AUTH=true` on the Convex deployment and `VITE_ENABLE_DEV_EMAIL_AUTH=true` in `.env.local`. Also set `ALLOWED_EMAIL`, `BETTER_AUTH_SECRET`, and `SITE_URL=http://localhost:3000` on Convex; `VITE_SITE_URL=http://localhost:3000` in `.env.local`. Notion OAuth vars are optional when dev email auth is on. Use「初回所有者として登録」once, then「メールでログイン」with the same email/password.
