...
export const setVisibleCalendars = ownerMutation({
  args: { connectionId: v.optional(v.id("calendarConnections")), calendarIds: v.array(v.string()) },
  handler: async (ctx, args) => {
    const connected = await setVisible(ctx, ctx.ownerId, args.calendarIds, args.connectionId);
    if (!connected) {
      throwDomain(
