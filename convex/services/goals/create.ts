...
export async function create(
  ...
  if (goal.type === "exam") {
    ...
    const examId = await ctx.db.insert("goals", toGoalDocument(goal, ownerId));
    await scheduleGoalSync(ctx, ownerId, [examId]);
    return examId;
  }
  ...
  const goalId = await ctx.db.insert(
    "goals",
    toGoalDocument({ ...goal, ...progress, scopeItemIds }, ownerId),
  );
  await scheduleGoalSync(ctx, ownerId, [goalId]);
  return goalId;
}
