  ...
  return (
    <Group gap={4} wrap="nowrap">
      ...
      <Text c="var(--cairn-muted-2)" ff={NUMERAL_FONT} size="xs">
        {previousWeekLabel(current, previous, unit)}
      </Text>
    </Group>
  );
}
...
type WeeklyReviewSummaryCardsProps = Pick<
  WeeklyReview,
  | "activeDays"
  | "confirmedMinutes"
  | "digest"
...
export function WeeklyReviewSummaryCards({
  activeDays,
  confirmedMinutes,
  digest,
  elapsedDays,
      ...
      <Grid.Col span={{ base: 12, sm: 4 }}>
        ...
        >
          ...
          {digest.isPartial && digest.countedThrough !== null ? (
            <Text c="var(--cairn-muted-2)" ff={NUMERAL_FONT} size="xs">
              {monthDayLabel(digest.countedFrom)}〜{monthDayLabel(digest.countedThrough)}{" "}
              を数えました（今日は数えません）
            </Text>
          ) : null}
        </SummaryCard>
