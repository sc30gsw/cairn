...
export function presetReviewCaption(windowStart: string, windowEnd: string): string {
  return `${windowStart} 〜 ${windowEnd}（今日と休養は含めない。直近${PRESET_REVIEW_WINDOW_DAYS}日）`;
}

export function suggestionCopy(suggestion: Suggestion, weekday: WeekdayRow | undefined): string {
...
export function suggestionLinkLabel(weekday: number): string {
  return `${weekdayLabel(weekday)}のプリセットを見る`;
}

export function weeklyTargetCopy(achieved: number, total: number): string {
  return `今週の週間ターゲットは ${achieved}/${total} 達成`;
}
