...
import type { BoardScheduleAllDayExpandAnchor } from "~/features/plan/components/board-schedule-all-day-expand";
import {
  blockFormValues,
  slotFormValues,
...
import { boardMoreLabel } from "~/features/plan/lib/board-schedule-layout";
...
export function useBoardScheduleUi({
  ...
  const [openedExternal, setOpenedExternal] = useState<PlanExternalEvent | null>(null);
  ...
  const baseEvents = [...toPlanScheduleEvents(blocks), ...toExternalScheduleEvents(externals)];
  ...
  function openEdit(block: PlanScheduleBlock) {
    collapseAllDayExpand();
