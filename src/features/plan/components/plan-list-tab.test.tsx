...
test("プランタブは日付カレンダー・Clock・予定入力を出し、祝日を出す", () => {
  ...
  expect(view.queryByRole("complementary", { name: "一日の時計" })).toBeNull();
...
test("Clock は計画プリセットと同じカード列にありカレンダーとは別", () => {
  const view = renderWithMantine(<PlanListTab />);
  const datePicker = view.getByLabelText("日付を選択");
  const addEvent = view.getByRole("button", { name: "予定を追加" });
  ...
  expect(follows(datePicker, addEvent)).toBe(true);
  expect(follows(addEvent, clockHeading)).toBe(true);
  expect(follows(clockHeading, templatesHeading)).toBe(true);
  expect(follows(templatesHeading, goalsHeading)).toBe(true);
  expect(follows(goalsHeading, obstaclesHeading)).toBe(true);
  expect(clockHeading.parentElement?.querySelector("svg")).not.toBeNull();
  expect(clockHeading.parentElement?.querySelector('[aria-label="日付を選択"]')).toBeNull();
