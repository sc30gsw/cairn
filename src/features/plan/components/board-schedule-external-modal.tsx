...
} from "~/features/plan/schemas/board-external-event-schema";
...
type ExternalModalProps = {
  external: PlanExternalEvent | null;
  onClose: () => void;
  onRemove: (externalId: PlanExternalEvent["_id"]) => Promise<MutationResult>;
...
function ExternalEventForm({
  ...
  onRemove,
  onUpdate,
}: Omit<ExternalModalProps, "external"> & { external: PlanExternalEvent }) {
  const formId = useId();
  const colorId =
