...
import { externalEventColor } from "~domain/googleCalendarColors";
...
export function toPlanScheduleEvents(blocks: readonly PlanScheduleBlock[]): ScheduleEventData[] {
  return blocks.map(
...
export function boardScheduleBlockIds(blocks: readonly PlanScheduleBlock[]): ReadonlySet<string> {
...
export function boardExternalEventId(eventId: string | number): PlanExternalEvent["_id"] {
  return boardScheduleEventSourceId(eventId).slice(
    BOARD_EXTERNAL_EVENT_PREFIX.length,
  ) as PlanExternalEvent["_id"];
}
...
export function movedScheduleRange(
  external: Pick<PlanExternalEvent, "endAt" | "startAt">,
  displayedStart: string | Date,
  newStart: string,
): Pick<PlanExternalEvent, "endAt" | "startAt"> {
  const offset =
...
export function toExternalScheduleEvents(
  ...
  return externals.map((external) => ({
    color: externalEventColor(external),
    end: external.endAt,
