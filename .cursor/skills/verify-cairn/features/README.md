...
- [Day log](./day-log.md) covers opening today, adding an ad-hoc record, confirming it, checking learning volume, 障害プラン (`更新` on existing rows), and 日 記録 from `この日に適用` / `適用を解除`.
- [Presets](./presets.md) covers 計画プリセット on `/plan` (default プラン), the `/presets` redirect to `/plan`, `この日の予定` occupancy beside `計画プリセットを追加`, create/edit/delete (name row `{name}を編集` plus chevron `{name}の詳細を開く`, row `削除` with confirm), `この日に適用` / `適用を解除`, shared event field order (項目 → タイトル → 開始・終了 → 優先度), and the forgotten-template switch.
- [Plan](./plan.md) covers `/plan` with no `?tab=` (プラン, `学習日` chrome through today+7 plus list `予定を追加` and collapsed `予定一覧`; Clock last as `何に時間を使ったか`; no month grid; `?date=` beyond +7 clamps), modal event forms with `TimeInput` start/end, future 予定-only storage until 日 opens, timed 予定 on chosen `スケジュール` without Clock (item name when title is empty; year popover `予定を追加`; schedule date picker unchanged), leftover `/board?tab=schedule` and `/presets` both to `/plan`, 計画プリセット occupancy entry, and the read-only 目標 card.
...
Selectors and preconditions were checked against `src/features/auth/components/login-screen.tsx` (title `学習ログ`), `src/features/auth/components/account-auth-form.tsx`, `src/lib/app-nav.ts` (`計画` → `/plan`; `MOBILE_PRIMARY` `日` / `ボード` / `計画` / `目標`), `src/routes/presets.tsx` (redirect search `{ tab: "plan" }`, location `/plan` after strip), `src/routes/board.tsx` (`tab=schedule` → `/plan`), `src/features/board/components/board-page.tsx` (kanban only), `convex/lib/jst.ts` (`planWeekAheadMaxDateJst` today+7), `src/components/learning-date-navigation.tsx` (`学習日` / `前の日` / `次の日` / `今日へ戻る`; optional `maxDateJst`), `src/lib/learning-date-picker-props.ts` (`maxDate` from `maxDateJst`), `src/features/plan/hooks/use-plan-view.ts` (`derivePlanView` clamps `?date=` beyond today+7), `src/features/plan/components/plan-page.tsx` / `plan-tabs.tsx` / `plan-clock.tsx` (`PLAN_CLOCK_HEADING` `何に時間を使ったか`), `src/features/plan/components/plan-list-tab.tsx` (`LearningDateNavigation` with `maxDateJst`, `PlanEventAddButton` `予定を追加`, `予定一覧` Collapse starts closed, empty `この日の予定はまだありません。`, Clock last peer Card), `src/features/plan/components/plan-templates-card.tsx` (header `PlanDayScheduleHeaderButton` `この日の予定`, `PlanTemplateAddButton`, name-row `{name}を編集`, chevron `{name}の詳細を開く`, visible `削除` with `{name}を削除` confirm, `この日に適用` / `適用を解除`, summary under title, preset rows 項目 → タイトル → 開始・終了 → 優先度, no `24:00に設定`), `src/features/plan/lib/plan-event-display-name.ts` (item name when `itemId` set), `convex/services/plan/templates.ts` / `convex/mutations/planTemplates/unapplyDate.ts` / `convex/services/plan/openDate.ts` (item-only materialize; unapply deletes linked rows), `src/features/plan/components/plan-day-schedule-list.tsx` (`PlanEventAddButton`, `PlanDayScheduleCollapsePanel` `DataList` `withDivider`, occupancy empty `この日の予定はまだありません。`), `src/features/plan/components/board-schedule-event-form.tsx` (modal field order, `TimeInput` 開始/終了, title hidden when item set), `src/features/plan/components/board-schedule-navigation-shared.tsx` (`日付を選択` via `planDatePickerProps`, schedule max unchanged), `src/features/plan/components/board-schedule-year-day-popover.tsx` (`{date}の予定` / `予定を追加`), `src/components/plan-goals-read-card.tsx`, `src/components/obstacle-section.tsx` (`更新` on existing rows; 日 and 計画), `src/features/today/components/day-board-tab.tsx` (`presets: []`), `src/features/onboarding/lib/setup-steps.ts` (`計画プリセットを登録する`; stepper href `/plan?tab=plan` then strip), `src/features/catalog/components/item-list.tsx` (`AddCategoryForm`), `src/features/today/components/adhoc-row-form.tsx` (initial `minutes: 20`), `src/features/methods/components/method-catalog-board.tsx` (heading `方法カタログ`), `src/features/board/components/board-kanban.tsx` (card-body `aria-label` `{item} の順序を変更`), `src/components/overflow-tooltip.tsx` (`EVENTS.touch`), `convex/services/days/openDay.ts` (future `days.open` no-op), and `src/features/today/hooks/use-day-page-date-jst.ts`. Consult graft before changing recipes when these controls change.
