...
- Several records can share the same item label. Scope the `{item} の操作` button to its card or choose the intended occurrence from a fresh snapshot. Duplicate cards show badge `1件目` or `2件目` beside the status badge. There is no count badge. Every tied plan window adds an orange `data-plan-caption` of `HH:MM〜HH:MM`, heavier than the dimmed category line. A lone card of an item has neither badge nor caption.
