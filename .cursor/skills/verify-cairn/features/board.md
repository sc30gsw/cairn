...
- `board-distinction` shows an orange caption (`data-plan-caption`) of `HH:MM〜HH:MM` when a 予定 is tied to that record. It is heavier than the dimmed category line. When the same item appears more than once, badges `N件目` and `N件` sit immediately left of the title. Those badges are not the caption. A single untied card has neither caption nor badges. The record itself does not store start or end.
...
- Several records can share the same item label. Scope the `{item} の操作` button to its card or choose the intended occurrence from a fresh snapshot. Duplicate cards show badges `1件目` / `2件目` and `2件` on the left of the title. A tied plan window adds an orange `data-plan-caption` of `HH:MM〜HH:MM`, heavier than the dimmed category line. A lone card of an item has neither badges nor caption.
