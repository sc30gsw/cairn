...
- `plan-create` there is no `予定を追加` on プラン. Empty days get 予定 from `この日に適用` after clicking the 計画プリセット name row (`検証計画プリセットを編集`; no visible `編集`). Event cards still open `予定を編集`. One-off timed create is `schedule-create`.
...
- **Create on プラン.** Assert `queryByRole('button', { name: '予定を追加' })` is empty. Create a 計画プリセット (see [presets.md](./presets.md)), click the name row `検証計画プリセットを編集` (not a visible `編集` button), then `この日に適用` on an empty day. Wait for the event card (example `検証予定`). Reload. Wait for heading `何に時間を使ったか`. Still on `プラン` with Clock last and that card. Click the card to open `予定を編集`. A title-only event does not paint a Clock sector. One-off timed create is **Create from a slot** or **Year entry** on スケジュール.
