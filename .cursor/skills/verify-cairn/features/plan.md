...
- **今日 resets the grid.** From that October URL, click `今日`. Assert `今日の計画` and month button `2026年9月` when today is 2026-09-21. Then click `getByRole('button', { name: '次', exact: true })` inside `getByLabel('日付を選択')`. Lead copy stays `今日の計画` while the month button becomes `2026年10月`. Click `今日` again and assert `2026年9月`. Do not use `[data-direction="next"]`: that selector matches both the button and its svg.
