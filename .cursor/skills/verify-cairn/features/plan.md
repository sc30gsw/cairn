...
- `plan-create` there is no `予定を追加` on プラン. Empty days get 予定 from `この日に適用` after clicking the 計画プリセット name row (`検証計画プリセットを編集`; no visible `編集`). Event cards still open modal `予定を編集`. One-off timed create is `schedule-create`.
- `plan-event-form` in modal `予定を追加` / `予定を編集` (card click or スケジュール slot), fields are 項目 → タイトル → 開始・終了 → 優先度. Start/end are Mantine `TimeInput` on the selected plan date. Title hides when an item is selected and saves empty; title is required when item is unset.
...
- **Create on プラン.** Assert `queryByRole('button', { name: '予定を追加' })` is empty. Create a 計画プリセット (see [presets.md](./presets.md)), click the name row `検証計画プリセットを編集` (not a visible `編集` button), then `この日に適用` on an empty day. Wait for the event card (example `検証予定`). Reload. Wait for heading `何に時間を使ったか`. Still on `プラン` with Clock last and that card. Click the card to open modal `予定を編集` (not inline fields). In that modal, combobox `項目` precedes textbox `タイトル`, then spinbutton/textbox `開始` / `終了`, then combobox `優先度`. Selecting an item hides `タイトル`. A title-only event does not paint a Clock sector. One-off timed create is **Create from a slot** or **Year entry** on スケジュール.
...
- **Year entry.** On スケジュール, choose `年表示に切り替え`. Click a day button (names look like `9月 21, 2026`). That opens a popover `{date}の予定` (example `2026-09-21の予定`); it does not switch to day view by itself. The popover button `予定を追加` opens dialog `予定を追加` (same form as a slot). That is a create path. プラン still has no `予定を追加`.
...
- After save on スケジュール, wait for dialog `予定を追加` to hide. `getByText('検証プラン')` matches the title field while the overlay is still open. プラン has no list button that opens that dialog. Year-view popover `{date}の予定` does have `予定を追加`; that locator is スケジュール-only.
