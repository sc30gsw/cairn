...
- **Automated recipe.** After launch/doctor/browser on a fresh run, `rtk proxy .cursor/skills/verify-cairn/bin/prove-catalog` performs the desktop recipe including signup, reload, independent day read, and empty add-field checks after create. Read `proof.json` for exact sub-feature scope. It does not create a day record.
