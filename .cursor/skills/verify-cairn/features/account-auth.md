...
- The chosen username `vfy_${CAIRN_VERIFY_RUN_ID}` does not already exist on this disposable Convex deployment. Shell examples expand the exported run ID.
...
- Sign-up is rate-limited (`/sign-up/email` max 3 per 60s). Sign-in is rate-limited (`/sign-in/email` max 5 per 60s). Username sign-in still uses the client `signIn.username` path. A unique `CAIRN_VERIFY_RUN_ID` avoids collisions; retries in the same minute can fail with a Japanese error under the form.
