...
- **Name and weekdays.** Run `rtk proxy playwright-cli -s="$SESSION" fill "getByLabel('プリセット名', { exact: true })" "検証プリセット"`, then `rtk proxy playwright-cli -s="$SESSION" click "getByRole('combobox', { name: '曜日', exact: true })"`. The field is a MultiSelect. From the new snapshot, click one or more free weekday options (used weekdays are labelled `（使用中）` and do not apply). Press Escape if the list stays open.
