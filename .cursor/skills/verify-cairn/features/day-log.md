...
- `day-group-same-item` keeps each record as its own Convex document. A lone row stays `RowEditor` (form `{項目}の記録`, combobox `{項目}のひとこと`). Two or more rows of the same item render as one item: one form, badge `{n}件 · 未完了{i} · 完了{c}` (no word `予定`), and `合計 N分`. Minutes is the sum of the rows and is read-only; confirm sends each row's own minutes. Group status is 見送り if any row is スキップ, else 未完了 if any row is 未着手 or 進行中, else 完了. 見送り skips every row in the group; 確定 confirms every row. ひとこと shows only when every row shares the same string; differing ひとこと stays on ボード. The day page does not show 予定 times. If any row is 確定, 共有文 includes that item once, with confirmed minutes summed.
...
- **Second view.** Reload `/`. Wait for a volume heading such as `50分` (or `45分` if you stopped after the two-row confirm) before capturing. A snapshot that only shows `読み込み中` is not proof. The same single `検証項目の記録` form remains, with the count badge and status `完了`.
...
- Two rows of the same item do not merge into one document, but the day page shows one form `{項目}の記録`. Shared ひとこと is one combobox `{項目}のひとこと`. Differing ひとこと hides that field. Badge is `{n}件 · 未完了{i} · 完了{c}` plus `合計 N分`. There is no word `予定` in that badge, no combobox `{項目} N件目`, and no visible label `ひとこと`. Confirming or skipping the group acts on every row. A lone row has no count badge and keeps its own 分数. Row ひとこと is a combobox (Mantine Autocomplete), not a textbox. The add field `その日限りのひとこと` stays a textbox. The group 分数 field is a disabled NumberInput; typing it is not a user path. 共有文 lists the item once with confirmed minutes summed. ボード still shows one card per record (`N件目`). There is no fold/Collapse on 日.
