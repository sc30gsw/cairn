...
- **Obstacles.** Fill `もし` with `検証もし` and the `なら` textbox (accessible name includes `なら`) with a concrete step such as `机に向かって金のフレーズを1 Unit だけ開く`. Click `障害プランを追加`. After reload, `検証もしのもし` still holds `検証もし`. The same plan also appears on `/plan?tab=plan`.
