...
## 改訂（2026-09-02）: レビューをナビタブへ昇格（#77）

§8.5 / §14-8 の「昇格させない」を撤回する。根拠だった `CONTEXT.md`「マイページ」_Avoid_「8番目のナビタブ」は、方法カタログが `/methods` として8本目のタブになった時点で前提を失った。所有者の判断で _Avoid_ の意図を「マイページをナビタブにすること」（本数の上限ではない）と確定し、`CONTEXT.md` を改訂した。

- デスクトップの右小口レールは `/review`（ラベル「レビュー」）を履歴の直後に置き、9本になる。
- モバイルは `MOBILE_PRIMARY` の4本（日 / ボード / 履歴 / 目標）を据え置き、レビューは「その他」メニューに入る（`docs/specs/pwa-mobile.md` の改訂節を参照）。
- 履歴ページと目標ページの週間ターゲット節にある `Anchor` は残す（週の締めの文脈からの入口として引き続き有効）。
- `weeklyTargetMiss` 通知のリンク先 `/review` は変更なし。
